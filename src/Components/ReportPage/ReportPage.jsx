...
const ReportPage = () => {
    ...
    const handleFormSubmit = async (values) => {
      //  setLoading(true);
        ...
        try {
            var api = await axios.post(import.meta.env.VITE_REACT_APP_API_REPORT_URL, {
                email: email,
                shorty: shorty_url,
            ...
            <Col span={24} style={{ minWidth: '100%' }}>
                <Card
                    style={{
                        ...
                        <h2>Report Link</h2>
                         <Alert
                      message={<><p><BugTwoTone twoToneColor="red" /> If you come across a Short Link that appears to be redirecting to a spam URL or being exploited for fraudulent purposes, please utilize this platform for reporting such instances.</p></>}
                      type="info"
                    ...
                    >
                        ...
                        >
                            <Input placeholder="Enter Short URL here" onChange={(e) => setFormValues({ ...formValues, shorty_url: e.target.value })} />
                        </Form.Item>
                        <Form.Item 
